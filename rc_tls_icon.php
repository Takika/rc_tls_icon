            ...
            foreach (array_values($tls_icon_headers) as $m_config) {
                ...
                if (preg_match_all("/$pattern/im", $Received, $items, PREG_PATTERN_ORDER)) {
                    if (isset($items[0][0])) {
                        ...
                        $this->icon_img .= '<img class="lock_icon" src="' . $icondir . $icon . '"' . 'title="'. htmlentities($data) .'" />';
                        $found = true;
                    }
                } else if (preg_match_all('/\[127.0.0.1\]|\[::1\]/im', $Received, $items, PREG_PATTERN_ORDER)) {
                    $this->icon_img .= '<img class="lock_icon" src="' . $icondir . 'blue_lock.png" title="' . $this->gettext('internal') . '" />';
                    $found = true;
                }
